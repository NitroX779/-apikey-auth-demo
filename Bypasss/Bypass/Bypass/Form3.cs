...
namespace Bypass
{
    public partial class Form3 : Form
    {
        ...
        private readonly string name = "Bypass";
        private readonly string ownerid = "NYVOikiir1";
        ...
        {
            string licenseKey = guna2TextBox1.Text;
            if (string.IsNullOrEmpty(licenseKey))
            {
                label2.Text = "Inserisci una license key";
            ...
            try
            {
                ...
                bool success = KeyAuthApp.login(licenseKey);
                
                if (success)
                {
                    label2.Text = "Autenticato con successo!";
                    label2.ForeColor = Color.Green;
                    
                    // Optionally store HWID
                    string hwid = GetHWID();
                    // You can send HWID to your server if needed
                }
                else
                {
                    label2.Text = "License key non valida";
                    label2.ForeColor = Color.Red;
                }
            }
            catch (Exception ex)
        ...
        private async void guna2ToggleSwitch1_CheckedChanged(object sender, EventArgs e)
        {
            if (guna2ToggleSwitch1.Checked)
            {
                ...
                using (HttpClient client = new HttpClient())
                {
                    try
                    {
                        HttpResponseMessage response = await client.GetAsync("https://apikey-auth-demo.onrender.com");
